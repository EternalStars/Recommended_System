...
def get_fragement(raw_data):
    init_list = []
    for i in range(0, len(raw_data)):
        j = 0
        tmp_dic = {}
        tmp_list = []
        for key in raw_data[i][2].keys():
            if j >= 6:
                break
            else:
                tmp_dic[key] = raw_data[i][2][key]
                j += 1
        tmp_list.append(raw_data[i][0])
        tmp_list.append(j)
...
if __name__ == "__main__":
    ...
    test_stat = get_fragement(train_stat)
    frag = get_fragement(train_stat)
    counter = 0
    for i in range(0, len(test_stat)):
        ...
        if i % 100 == 99:
            ...
            files.writepkl('data/rmse_stat%d.pkl' % counter, test_stat[counter * 100:counter * 100 + 100])
            files.writepkl('data/rmse_origin%d.pkl' % counter, frag[counter * 100:counter * 100 + 100])
            counter += 1
